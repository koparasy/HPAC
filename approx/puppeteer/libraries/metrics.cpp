...
double computeVALUE(double *correct, double *test, size_t elements){
  if (elements != 1){
    printf("Value is applicable only with a single value output");
    exit(-1);
  }
  return test[0];
}
...
double computeRE(double *correct, double *test, size_t elements){
    double error =0.0;
    double sqrtDiff= 0;
    double p2Diff = 0.0;
...
double computeQuality(double *correct, double *test, size_t elements, char *metric){
    ...
    else if ( strcmp(metric,"VALUE") == 0 )
      return computeVALUE(correct, test,elements);
    ...
    else if (strcmp( metric, "RE") == 0 )
        return computeRE(correct, test, elements);
