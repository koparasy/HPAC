    ...
    double * times, int num_splits, double *bounds, char **names)
{
  ...
  for(k=1; k<max_iter && normr > tolerance; k++ )
  {
    ...
    else
    {
      oldrtrans = rtrans;
      //This is par_2
      snprintf(label_name,1000,"%s_%d_DDOT_1",names[bin], bin); 
#pragma approx label(label_name) petrubate(out) out(rtrans)
      ddot (nrow, r, r, &rtrans, t4); 
      double beta = rtrans/oldrtrans;
      //This is par_5
      snprintf(label_name,1000,"%s_%d_WAXPBY_2",names[bin], bin); 
#pragma approx label(label_name) petrubate(out) out(p[0:nrow])
    ...
    snprintf(label_name,1000,"%s_%d_SPARSEMV",names[bin], bin); 
#pragma approx label(label_name) petrubate(out) out(Ap[0:nrow])
    HPC_sparsemv(p, Ap); // 2*nnz ops
    double alpha = 0.0;
    //This is par(1)
    snprintf(label_name,1000,"%s_%d_DDOT_2",names[bin], bin); 
#pragma approx label(label_name) petrubate(out) out(alpha)
    ddot(nrow, p, Ap, &alpha, t4);
    alpha = rtrans/alpha;
    snprintf(label_name,1000,"%s_%d_WAXPBY_3",names[bin], bin); 
#pragma approx label(label_name) petrubate(out) out(x[0:nrow])
    waxpby(nrow, 1.0, x, alpha, p, x);// 2*nrow ops
    snprintf(label_name,1000,"%s_%d_WAXPBY_4",names[bin], bin); 
#pragma approx label(label_name) petrubate(out) out(r[0:nrow])
...
{
  *exp =  (int) std::log10(std::fabs(arg));
}
