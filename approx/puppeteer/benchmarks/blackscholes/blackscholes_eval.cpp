        ...
        fptype time, int otype, float timet )
{
    ...
    xTime = time;
//@APPROX LABEL("SQRT") APPROX_TECH(PETRU) OUT(xSqrtTime)
    xSqrtTime = fastsqrt(xTime);

    temp = sptprice / strike;

//@APPROX LABEL("LOG") APPROX_TECH(PETRU) OUT(logValues)
    logValues = fastlog( sptprice / strike );
...
//@APPROX LABEL("CNDF1") APPROX_TECH(PETRU) OUT(NofXd1)
    NofXd1 = CNDF( d1 );

//@APPROX LABEL("CNDF2") APPROX_TECH(PETRU) OUT(NofXd2)
    NofXd2 = CNDF( d2 );

    temp = -(rate*time);

//@APPROX LABEL("EXP") APPROX_TECH(PETRU) OUT(FutureValueX)
